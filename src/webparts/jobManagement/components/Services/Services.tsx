...
const Services = (props: any): JSX.Element => {
  ...
  const [providerDropDown, setProviderDropDown] = React.useState<
    IDropdownOption[]
  >([]);
  const [clientrDropDown, setClientDropDown] = React.useState<
    IDropdownOption[]
  >([]);
  const [contructorDropDown, setContructorDropDown] = React.useState<
    IDropdownOption[]
  >([]);
  ...
  const getProviderData = async () => {
    await sp.web.lists
      .getByTitle("ProviderList")
      .items.select("id,ProviderName,Status")
      .filter("Status eq " + "'" + approve + "'")
      .get()
      .then((items) => {
        if (items) {
          let ProviderDrop = [];
  ...
  const getClientData = async () => {
    await sp.web.lists
      .getByTitle("Client")
      .items.select("id,ClientName,Status")
      .filter("Status eq " + "'" + approve + "'")
      .get()
      .then((items) => {
        if (items) {
  ...
  const getContructorData = async () => {
    await sp.web.lists
      .getByTitle("Contructor")
      .items.select("id,ContrctName,Status")
      .filter("Status eq " + "'" + approve + "'")
      .get()
      .then((items) => {
        if (items) {
  ...
  const getMasterData = async (value: any) => {
    ...
    if (value.items.key) {
      await sp.web.lists
        .getByTitle(value.listName)
        .items.select("*")
        .getById(value.items.key)
        .get()
        .then((data) => {
          let Data: IData = {
          ...
          if (Service.RecurrenceDates.length) {
            for (let i = 0; i < Service.RecurrenceDates.length; i++) {
              let Json = {
                ...
                ServiceName: Service.serviceName ? Service.serviceName : "",
                ServiceDate: Service.RecurrenceDates[i]
                  ? Service.RecurrenceDates[i]
                  : "",
                Notes: Service.serviceNotes ? Service.serviceNotes : "",
                ProviderDetailsId: providerData.Id ? providerData.Id : null,
                ClientDetailsId: clientData.Id ? clientData.Id : null,
              ...
              await sp.web.lists
                ...
                .then((item) => {})
                .catch((error) =>
                  errorFunction("add services  recurrence data", error)
                );
            }
          } else {
            let Json = {
  ...
  const createFolder = async (itemId) => {
    if (itemId) {
      await sp.web.rootFolder.folders
        .getByName(props.list.libraryName)
        .folders.addUsingPath(itemId.toString(), true)
        .then(async (result) => {
          if (Service.files.length > 0) {
            for (let i = 0; i < Service.files.length; i++) {
              await sp.web
                .getFolderByServerRelativePath(result.data.ServerRelativeUrl)
                .files.addUsingPath(Service.files[i].name, Service.files[i], {
                  Overwrite: true,
                })
                .then((res) => {
                  props.setChange({
      ...
      <div>
        <div className={styles.serviceContainer}>
          <div className={styles.serviceContent}>
            <div className={styles.serviceBox}>
              <Dropdown
                ...
                onChange={(e, item) =>
                  setSelectDrop({ listName: "ProviderList", items: item })
                }
              />
            </div>
          </div>
          ...
          <div className={styles.serviceContent}>
            ...
            <div className={styles.serviceBox}>
              <TextField
                value={providerData.FirstAddress}
                label="FirstAddress"
                disabled={true}
              />
            </div>
            <div className={styles.serviceBox}>
              <TextField
                value={providerData.SecondAddress}
                label="SecondAddress"
                disabled={true}
              />
            </div>
        ...
        <div className={styles.serviceContainer}>
          <div className={styles.serviceContent}>
            <div className={styles.serviceBox}>
              <Dropdown
                ...
                onChange={(e, item) =>
                  setSelectDrop({ listName: "Client", items: item })
                }
              />
            </div>
          </div>
          ...
          <div className={styles.serviceContent}>
            ...
            <div className={styles.serviceBox}>
              <TextField
                value={clientData.SecondAddress}
                label="SecondAddress"
                disabled={true}
              />
            </div>
        ...
        <div className={styles.serviceContainer}>
          <div className={styles.serviceContent}>
            <div className={styles.serviceBox}>
              <Dropdown
                ...
                onChange={(e, item) =>
                  setSelectDrop({ listName: "Contructor", items: item })
                }
              />
            </div>
          </div>
          ...
          <div className={styles.serviceContent}>
            ...
            <div className={styles.serviceBox}>
              <TextField
                value={contructorData.FirstAddress}
                label="FirstAddress"
                disabled={true}
              />
            </div>
            <div className={styles.serviceBox}>
              <TextField
                value={contructorData.SecondAddress}
                label="SecondAddress"
                disabled={true}
              />
            </div>
          ...
          <div className={styles.serviceContent}>
            <div className={styles.serviceBox}>
              <Dropdown
                ...
                onChange={(e, item) =>
                  setService({ ...Service, serviceName: item.text })
                }
              />
            </div>
            <div className={styles.serviceBox}>
              <DatePicker
                ...
                onSelectDate={(e) =>
                  setService({ ...Service, serviceDate: new Date(e) })
                }
              />
            </div>
            <div>
            ...
            <div className={styles.serviceBox}>
              <TextField
                label="Notes"
                onChange={(e, text) =>
                  setService({ ...Service, serviceNotes: text })
                }
                multiline
                style={{ resize: "none" }}
              />
            ...
            <div>
              ...
              <Checkbox
                checked={Service.Recurrence}
                label={Service.Recurrence ? "Yes" : "No"}
                onChange={(e, text) => handleRecurrence(text)}
              />
            </div>
          </div>
          <div>
              ...
              <div className={styles.serviceContent}>
                <div className={styles.serviceBox}>
                  <Dropdown
                    ...
                    disabled={!Service.Recurrence}
                    onChange={(e, item) =>
                      setService({ ...Service, RecurrenceType: item.text })
                    }
                  />
                </div>
                <div className={styles.serviceBox}>
                  <DatePicker
                    ...
                    disabled={!Service.Recurrence}
                    onSelectDate={(e) =>
                      setService({ ...Service, StartDate: new Date(e) })
                    }
                  />
                </div>
                <div className={styles.serviceBox}>
                  <DatePicker
                    ...
                    disabled={!Service.Recurrence}
                    onSelectDate={(e) =>
                      setService({ ...Service, EndDate: new Date(e) })
                    }
                  />
                </div>
              </div>
